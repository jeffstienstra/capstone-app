...
# parkCode = nps[:data][index][:parkCode]
# fullName = nps[:data][index][:fullName]
# description = nps[:data][index][:description]
# activities_raw = nps[:data][index][:activities]
# activities = nps[:data][index][:activities][index2][:name]
# states = nps[:data][index][:states]
# phoneNumbers = nps[:data][index][:contacts][:phoneNumbers][0][:phoneNumber]
# emailAddresses = nps[:data][0][:contacts][:emailAddresses][0][:emailAddress]
# cost (general admission)= nps[:data][0][:entranceFees][0][:cost]
# title (admission type) = nps[:data][0][:entranceFees][0][:title]
# description (for admission type) = nps[:data][0][:entranceFees][0][:description]
# entrancePasses (typical annual pass name) = nps[:data][0][:entrancePasses][0][:title]
# entrancePasses (typical annual pass cost) = nps[:data][0][:entrancePasses][0][:cost]
# directions = nps[:data][0][:directionsInfo]
# directionsURL = nps[:data][0][:directionsUrl]
# addresses = nps[:data][0][:addresses]
...
p nps[:data][0][:fullName]
p nps[:data][0][:description]

# collect activity name only, not activity id
activities = []
i = 0
while i < activities_raw.length
  activities << activities_raw[i][:name]
  i += 1
end
puts activities
